...
async def on_ready():
    ...
    # Try to restore panel in last used channel
    for channel_id in ALLOWED_CHANNELS:
        ...
        if ch:
            ...
            try:
                ...
                for msg in messages:
                    if msg.author == bot.user and msg.content.startswith("Choose a panel:"):
                        panel_message = msg
                        break
            except:
                pass
...
async def addm(ctx, *, text):
    for line in text.split("\n"):
        if line.strip():
            add_panel_line("mina", line.strip())
...
async def adds(ctx, *, text):
    for line in text.split("\n"):
        if line.strip():
            add_panel_line("sora", line.strip())
...
async def addb(ctx, *, text):
    for line in text.split("\n"):
        if line.strip():
            add_panel_line("brnzel", line.strip())
    await ctx.send("Added to Brnzel panel.")

@bot.command()
@is_bot_staff()
async def delm(ctx, number: int):
    removed = del_panel_line("mina", number-1)
    if removed:
...
async def dels(ctx, number: int):
    removed = del_panel_line("sora", number-1)
    if removed:
...
async def delk(ctx, number: int):
    removed = del_panel_line("kay", number-1)
    if removed:
...
async def delb(ctx, number: int):
    removed = del_panel_line("brnzel", number-1)
    if removed:
